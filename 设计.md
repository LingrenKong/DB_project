...
* 联系：
  ...
  * 用户信息修改：修改类型、修改项、涉及数值、原始数值、最终数值
  * 图书管理：操作类型、操作数值、原始数值、最终数值



### 转换为关系模型

| 借阅者   | 学生/教师号       | 姓名         | 密码         | 用户类型   | 在借书数量   | 备注     | 是否已经被禁用 |
| -------- | ----------------- | ------------ | ------------ | ---------- | ------------ | -------- | -------------- |
| 字段名   | Uno               | Uname        | Upassword    | Utype      | Uget         | more     | Urestrict      |
| 字段类型 | char              | nvarchar     | nvarchar     | nvarchar   | int          | nvarchar | bit            |
| 限制     | 主键，11位        | 非空，最长10 | 非空，最长20 | 最长10     | 非负，非空   | 2000     | 非空           |
| 备注     | 如S/T/U2018201661 | 真名字       | 初始化学号   | 有限的几种 | 业务检查即可 | 初始为空 | 0代表否        |



| 图书     | 书号       | 书名          | 简介     | 类型     | 作者     | 出版社   | 价格     | 剩余数量 | 备注     | 是否下架 |
| -------- | ---------- | ------------- | -------- | -------- | -------- | -------- | -------- | -------- | -------- | -------- |
...
| 管理员Admin | 账号         | 密码      | 权限类型         | 备注     |
| ----------- | ------------ | --------- | ---------------- | -------- |
| 字段名      | Ano          | Apassword | Atype            | more     |
| 字段类型    | char         | nvarchar  | nvarchar         | nvarcahr |
| 限制        | 主键，20以内 | 最长20    | 最长10           | 2000以内 |
| 备注        |              |           | 不同权限不同效果 |          |
...
ALTER TABLE borrow ADD CONSTRAINT C1 FOREIGN KEY (userid) REFERENCES Userlist(id)
ALTER TABLE borrow ADD CONSTRAINT C2 FOREIGN KEY (bookid) REFERENCES book(id)
alter table borrow drop column id
alter table borrow add id int identity(1,1)
```

由于SQL server的问题，所以暂时没能成功创建外键
...
- [ ] 允许用户输入参数，对数据库进行汇总统计，用html table显示结果
